...
function isEmailValid(email) {
  ...
  if (email.value.length !== 0 && emailRegExp.test(email.value)) {
    ...
    addErrorMessage(
      'You must enter a valid email. Example: "name@domain.com".',
    );
    makeInvalid(email);
    return false;
  }
...
function isCountryValid(country) {
  ...
  if (
    ...
    addErrorMessage(
      "You must choose a listed country: Canada, Mexico, or the United States.",
    );
    makeInvalid(country);
    return false;
  }
...
function isPostalCodeValid(country, postalCode) {
  ...
  if (
    ...
    addErrorMessage(
      "You must enter a valid postal code for your selected country.",
    );
...
function isPasswordValid(password) {
  ...
  if (password.value.length !== 0 && passwordRegExp.test(password.value)) {
    makeValid(password);
    return true;
  } else {
    addErrorMessage(
      "You must enter a valid password. It must: be at least 8 characters long and contain at least one upper case letter, lower case letter, number, and symbol.",
    );
...
function isConfirmPasswordValid(password, confirmPassword) {
  if (
    ...
    addErrorMessage(
      "You must confirm your password by reentering the same password as above.",
    );
...
// event listeners for each field that checks validity of input live
