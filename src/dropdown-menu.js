...
document.documentElement.addEventListener("click", (button) => {
  ...
  let currentMenu = dropdownMenus.filter(
    (word) => button.target.closest(".dropdown-menu") === word,
  );
  ...
  let currentWindow = windows.filter(
    (word) => button.target.closest(".window") === word,
  );
